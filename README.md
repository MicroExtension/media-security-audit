...
- guarded HTTP headers audit now records per-target JSON evidence and checks HSTS, CSP, clickjacking, MIME sniffing, referrer, permissions, HTTPS, and platform disclosure posture
...
same inventory can be filtered by search text, client, or status, then
downloaded from the page as CSV, JSON, or Markdown. Active filters are
summarized on the page with shortcuts back to filters, inventory, and
downloads.
...
counts, and any missing, mismatched, or unexpected file details. The same
verification is available from the CLI and as JSON or Markdown downloads for
copy/handoff checks.
