...
def create_web_app(
    ...
    @app.get("/pilot/bundle-index.md", dependencies=protected)
    def pilot_bundle_index_markdown() -> Response:
        system_view = build_system_status(data_dir, reports_dir, settings)
        readiness_items = build_pilot_readiness_items(store, reports_dir, system_view)
    ...
    @app.get("/pilot/bundle-index.json", dependencies=protected)
    def pilot_bundle_index_json() -> Response:
        system_view = build_system_status(data_dir, reports_dir, settings)
        readiness_items = build_pilot_readiness_items(store, reports_dir, system_view)
        export = build_pilot_bundle_index_json_export(readiness_items)
        return Response(
            content=export.content,
            media_type=export.media_type,
            headers={"Content-Disposition": f'attachment; filename="{export.filename}"'},
    ...
    @app.get("/pilot/delivery-receipt.md", dependencies=protected)
    def pilot_delivery_receipt_markdown() -> Response:
        system_view = build_system_status(data_dir, reports_dir, settings)
        readiness_items = build_pilot_readiness_items(store, reports_dir, system_view)
        export = build_pilot_delivery_receipt_export(readiness_items)
        return Response(
            content=export.content,
            media_type=export.media_type,
            headers={"Content-Disposition": f'attachment; filename="{export.filename}"'},
    ...
    @app.get("/pilot/attention.json", dependencies=protected)
    def pilot_attention_json() -> Response:
        system_view = build_system_status(data_dir, reports_dir, settings)
        readiness_items = build_pilot_readiness_items(store, reports_dir, system_view)
        export = build_pilot_attention_json_export(readiness_items)
        return Response(
            content=export.content,
            media_type=export.media_type,
            headers={"Content-Disposition": f'attachment; filename="{export.filename}"'},
    ...
    def pilot_evidence_bundle() -> Response:
        system_view = build_system_status(data_dir, reports_dir, settings)
        readiness_items = build_pilot_readiness_items(store, reports_dir, system_view)
    ...
    @app.get("/pilot/bundle-verification.json", dependencies=protected)
    def pilot_evidence_verification_json() -> Response:
        system_view = build_system_status(data_dir, reports_dir, settings)
        readiness_items = build_pilot_readiness_items(store, reports_dir, system_view)
        verification = build_pilot_evidence_verification_json(readiness_items)
        return Response(
            content=verification.content,
            media_type=verification.media_type,
            headers={"Content-Disposition": f'attachment; filename="{verification.filename}"'},
    ...
    @app.get("/missions/{mission_id}/readiness/{export_format}", dependencies=protected)
    def mission_readiness_download(
        mission_id: str,
    ...
    @app.get("/missions/{mission_id}/export-manifest/{export_format}", dependencies=protected)
    def mission_export_manifest_download(
        mission_id: str,
        export_format: MissionExportManifestFormat,
    ) -> Response:
        try:
            store.get_mission(mission_id)
            export = build_mission_export_manifest_export(
                mission_id,
                reports_dir,
                export_format,
            )
