...
- keep session client briefs concise, remediation-led, and aligned with report/export readiness
...
- keep post-test review centers focused on executions, CVE/KEV validation, critical findings, remediation, and delivery
...
Use `bash scripts/debian-vm-release-candidate.sh` after pilot closeout to write
the final V1 release candidate go/no-go report under `reports/release-candidate`
...
- local Pilot page summarizes total pilot evidence archive bytes before download
- local Pilot page keeps archive byte summaries aligned with manifest generation
